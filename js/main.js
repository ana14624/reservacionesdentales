...
let db = prodb("Productdb", {
  products: `++id, name, phone, dayhour`
});
...
btncreate.onclick = event => {
  ...
  let flag = bulkcreate(db.products, {
    name: proname.value,
    phone: phone.value,
    dayhour: dayhour.value
  });
  proname.value = phone.value = dayhour.value = "";
    ...
    if (data) {
      createEle("tr", tbody, tr => {
        for (const value in data) {
          createEle("td", tr, td => {
            td.textContent = data.dayhour === data[value] ? ` ${data[value]}` : data[value];
          });
        }
        createEle("td", tr, td => {
          createEle("i", td, i => {
